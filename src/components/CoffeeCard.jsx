      ...
      <div className="card card-side bg-[#ECEAE3] hover:shadow-xl h-full">
        ...
        <div className="card-body flex-row justify-between p-0">
          <div className="flex flex-col justify-center gap-2 text-xl font-sans">
            ...
            <h2 className="flex gap-1 font-semibold">
              Chief:<span className="font-normal">{coffee.chief}</span>
            </h2>
            <h2 className="flex gap-1 font-semibold">
          ...
          <div className="card-actions justify-end pr-5">
            <div className="join join-vertical space-y-2 mt-1 sm:mt-8">
              <Link to={`/coffeeDetails/${coffee._id}`}>
                <button className="btn join-item bg-[#E3B577] hover:bg-[#c18f4e]">
                  <MdRemoveRedEye size={20} color="white" />
                </button>
              </Link>
              <Link to={`/updateCoffee/${coffee._id}`}>
                <button className="btn join-item bg-black/70 hover:bg-black">
                  <FaPencil size={20} color="white" />
                </button>
              </Link>
              <button
                onClick={() => deleteHandler(coffee._id)}
                className="btn join-item btn-error"
              >
                <ImBin2 size={20} color="white" />
              </button>
            </div>
          </div>
        </div>
